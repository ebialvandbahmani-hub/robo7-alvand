...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if onb: # فرآیند ثبت‌نام
        if onb.get("step") == "name":
            onb["data"]["name"] = text
            onb["step"] = "phone"
            phone_btn = KeyboardButton("📱 ارسال شماره تماس", request_contact=True)
            await update.message.reply_text("شماره تماس خود را بفرستید:", reply_markup=ReplyKeyboardMarkup([[phone_btn]], resize_keyboard=True))
            return
...
async def main():
    ...
    finally: await app.updater.stop()
